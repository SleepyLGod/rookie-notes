...
关于这个函数库的更详细内容在[文档的稍后部分](contracts-guide.md#FixedPoint)。
...
如果有新的流动性变化需要收取协议费。 你可以在 [本文后面](contracts-guide.md#Math)看到平方根函数。
...
当流动资金提供者为资金池增加流动资金时，将会调用此函数。 它将产生额外的流动池 代币作为奖励。 在 [外围合约](contracts-guide.md#UniswapV2Router02)中增加流动性后调用这个函数，以确保二者在同一交易中（因此其他人都不能提交向合法所有者要求新流动资金的交易）。
...
当流动资金被提取且相应的流动池代币需要被销毁时，将调用此函数。 还需要[从一个外围账户](contracts-guide.md#UniswapV2Router02)调用。
...
此函数也应该从[外围合约](contracts-guide.md#UniswapV2Router02)调用。
...
这些函数转发元交易，通过[许可证机制](contracts-guide.md#UniswapV2ERC20)使没有以太币的用户能够从流动池中提取资金。
...
这些函数仅仅是调用 [UniswapV2Library 函数](contracts-guide.md#uniswapV2library)的代理。
